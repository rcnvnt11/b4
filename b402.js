...
async function start() {
    ...
    /* ------------ JWT INPUT (MENU 2 ONLY) ------------ */
    console.log("\n=== B402 SPAM MINT ===");
    console.log("JWT manual mode aktif (menu 2 only)\n");

    const JWT_INPUT = prompt("Paste JWT here: ").trim();
    if (!JWT_INPUT) {
        console.log("❌ ERROR: JWT empty");
        return start(); // ulang menu
    }
    ...
    /* ------------ MAIN EXECUTION (IDENTIK, ONLY JWT_CHANGED) ------------ */
